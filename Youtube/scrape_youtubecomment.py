...
def download_comments(youtube_id, sort_by=SORT_BY_RECENT, sleep=.1):
    ...
    while continuations:
        ...
        for comment in reversed(list(search_dict(response, 'commentRenderer'))):
            ...
            yield [cid,text,time,author,channel,votes,photo,heart]
...
def parse_ytcomment(youtube_id,csv_file):
    try:
        ...
        with open(csv_file, 'a+',newline='',encoding='utf-8-sig') as fp:
            ...
            for comment in download_comments(youtube_id):
                w.writerow([youtube_id,comment[0][0],comment[1][0],comment[2][0],comment[3][0],\
                            comment[4][0],comment[5][0],comment[6][0],comment[7]])
                count += 1
                sys.stdout.write('Downloaded %d comment(s)\r' % count)
                sys.stdout.flush()
